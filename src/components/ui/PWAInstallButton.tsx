...
import { Download, Smartphone, Check, Sparkles, X, Share2, PlusSquare } from 'lucide-react';
...
export default function PWAInstallButton({ variant = 'sidebar' }: PWAInstallButtonProps) {
  ...
  const handleInstallClick = async () => {
    ...
    if (deferredPrompt) {
      ...
      // Fallback for browsers when prompt is not yet ready or custom install
      setShowIOSModal(true);
    }
  };

  // Render for Sidebar (Desktop & Mobile)
  if (variant === 'sidebar') {
    return (
      <>
          ...
          <button
            ...
            className={`w-full p-3 rounded-xl transition-all border flex items-center gap-3 text-left group ${
              isInstalled
                ...
                : 'bg-gradient-to-r from-[#182030] to-[#121620] hover:from-[#1e283c] hover:to-[#182030] border-[#2a3a50] text-[#e2e6ed] shadow-lg shadow-black/20'
            }`}
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${
              isInstalled 
                ? 'bg-[#1a2030] text-[#6ea87a]' 
                : 'bg-[#2a3850] group-hover:bg-[#3d5a80] text-[#b89860]'
            }`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                {!isInstalled && (
                  <span className="px-1.5 py-0.2 bg-[#b89860]/20 text-[#c8a870] text-[0.55rem] font-black rounded uppercase">
                    PWA
                  </span>
                )}
              </div>
              <p className="text-[0.65rem] text-[#5a6270] truncate">
        ...
        {/* Modal for iOS / Browser Install Instructions */}
        {showIOSModal && (
          <div className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-[#121620] border border-[#2a3040] rounded-2xl max-w-sm w-full p-5 shadow-2xl space-y-4 animate-in zoom-in-95">
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => setShowIOSModal(false)} className="text-[#5a6270] hover:text-[#e2e6ed]">
                  <X className="w-4 h-4" />
                </button>
              </div>

              <div className="space-y-2.5 text-xs text-[#8899aa] bg-[#0c0f17] p-3.5 rounded-xl border border-[#1e2330]">
                <p className="font-semibold text-[#e2e6ed]">Cara Pasang di iPhone / iPad (Safari):</p>
                <div className="flex items-center gap-2.5 text-[0.75rem]">
                  ...
                  <span>Ketuk tombol <strong>Share / Bagikan</strong> <Share2 className="w-3.5 h-3.5 inline mx-1 text-[#7a8a9a]" /> di bilah navigasi Safari.</span>
                </div>
                <div className="flex items-center gap-2.5 text-[0.75rem]">
                  <span className="w-5 h-5 rounded-full bg-[#1a2030] text-[#7a8a9a] font-bold flex items-center justify-center shrink-0">2</span>
                  <span>Scroll ke bawah & pilih <strong>Tambahkan ke Layar Utama</strong> (Add to Home Screen <PlusSquare className="w-3.5 h-3.5 inline mx-1 text-[#7a8a9a]" />).</span>
                </div>
                <div className="flex items-center gap-2.5 text-[0.75rem]">
              ...
              <div className="space-y-1 text-[0.7rem] text-[#5a6270]">
                <p>💡 Untuk Chrome / Edge Desktop: Klik ikon <strong>Install</strong> di bagian kanan address bar browser.</p>
              </div>

              <button
                type="button"
                onClick={() => setShowIOSModal(false)}
                className="w-full py-2.5 bg-[#3d5a80] hover:bg-[#4a6d8c] text-[#e2e6ed] text-xs font-bold rounded-xl transition-all"
              >
                Mengerti
              </button>
            </div>
      ...
      <div 
        ...
        className={`glass-card rounded-2xl p-4 sm:p-5 flex flex-col justify-between group relative overflow-hidden transition-all duration-200 cursor-pointer border ${
          isInstalled 
            ...
            : 'border-[#2a3a50] bg-gradient-to-br from-[#151d2a] to-[#0e131d] hover:border-[#4a6d8c] text-slate-100 shadow-md'
        }`}
      >
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            {!isInstalled && (
              <span className="px-2 py-0.5 rounded-full bg-[#3d5a80]/30 border border-[#4a6d8c]/40 text-[#c8d4e0] text-[0.65rem] font-extrabold uppercase tracking-wider">
                Install PWA
              </span>
            )}
          </div>
          <h3 className="font-bold text-xs sm:text-sm text-[#e2e6ed] group-hover:text-white transition-colors">
        ...
        <div className="mt-3 pt-2.5 border-t border-[#1e2330] flex items-center justify-between text-[0.65rem] text-[#7a8a9a]">
          ...
          <span className="text-[#b89860] font-semibold">{isInstalled ? 'Terpasang' : 'Pasang Sekarang →'}</span>
        </div>
      </div>
